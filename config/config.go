...
const (
	LayoutModeFull   LayoutMode = "full"   // Standard desktop layout
	LayoutModeMobile LayoutMode = "mobile" // Small screen layout
	LayoutModeAuto   LayoutMode = "auto"   // Auto-detect based on terminal size
)
...
func (c *Config) GetEffectiveLayoutMode(terminalWidth int) LayoutMode {
	if c.LayoutMode != LayoutModeAuto {
		return c.LayoutMode
	}
